...
export const OrganizationDataTable: FunctionComponent = () => {
  ...
  return (
    <>
      <TrainingOrgLspContext.Provider value={{ lspName, setLspName, setTrainingIsOpen, trainingIsOpen }}>
                ...
                {({ open }) => (
                  <div className="">
                    <div className="flex items-center justify-between">
                      <Disclosure.Button className="flex items-center justify-between w-full py-2">
                        <div className="text-start">
                          <span className="font-medium text-indigo-700 text-md ">Awards & Recognitions</span>
                          {/* <p className="text-sm text-gray-500">{exp.subjectMatter}</p> */}
                        </div>
                        <svg
                          width="24"
                    ...
                    <Disclosure.Panel className="" as="ul">
                      {allLsp?.awards && allLsp?.awards.length > 0 ? (
                        allLsp?.awards.map((award: { name: string }, index: number) => {
                          return (
                            <li key={index} className="pl-2 space-y-3 text-sm text-gray-700">
                              ✓ {award.name}
                            </li>
                        ...
                        <div className="flex w-full h-[2rem] ">
                          <span className="pl-2 space-y-3 text-sm text-gray-400">✗ No Awards & Recgonitions Found</span>
                        </div>
                      )}
                    </Disclosure.Panel>
                  </div>
