...
export const CreateProjectsImplemented: FunctionComponent = () => {
  const {
    register,
    setValue,
    handleSubmit,
  ...
  return (
    <>
      <form id="createProjectsImplementedForm" key="createProjectsImplementedForm" onSubmit={handleSubmit(onSubmit)}>
        ...
        {projects.length === 0 ? (
          <div
            className={`flex items-center justify-center w-full border-2 border-dashed rounded-lg ${
              errors?.projects ? "bg-red-100 border-red-600" : "bg-gray-50/50"
            } `}
          >
            <div className="py-4">
              ...
              >
                {errors?.projects ? (
                  <span className="text-red-700">Add at least one project details</span>
                ) : (
                  <span className="text-gray-700">Add project details</span>
                )}
              </h3>
            </div>
          </div>
          ...
          <ul className="space-y-2">
            {projects.map((item, index) => (
              <div
                key={index}
                className="grid grid-cols-12 text-sm border-l-4 border-r rounded-r border-l-purple-400 border-y"
              >
                      ...
                      <Tooltip.Trigger asChild>
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (!isEmpty(newProjects)) {
                              setValue("projects", newProjects);
                            } else setValue("projects", undefined!);
