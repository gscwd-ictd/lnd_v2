...
export const DeleteEmployee: FunctionComponent<DeleteEmployeProp> = (data) => {
  const [open, setOpen] = useState(false);
  const setEmployees = useTabStore((state) => state.setEmployees);

  const deleteEmployeeTag = async (data: DeleteEmployeProp) => {
    ...
    try {
      ...
      if (result) {
        const { data } = await axios.get(`${url}/employee-tags/tag/${tagId}`);
