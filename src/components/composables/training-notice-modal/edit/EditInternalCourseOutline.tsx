...
export const EditInternalCourseOutline: FunctionComponent = () => {
  ...
  const [selectedLspSource, setSelectedLspSource] = useState<"internal" | "external">("internal");
  ...
  useEffect(() => {
    if (!isEmpty(selectedFacilitators)) {
      ...
      if (selectedFacilitators.filter((f) => f.type === "individual" && f.source === "external").length > 0) {
        setOrganizationTabIsLocked(true);
        setIndividualInternalTabIsLocked(true);
      }
      if (selectedFacilitators.filter((f) => f.type === "organization").length > 0) {
        setIndividualExternalTabIsLocked(true);
        setIndividualInternalTabIsLocked(true);
      }
  ...
  useEffect(() => {
    setValue("courseTitle", { courseTitle: selectedTrainingDesign.courseTitle, id: selectedTrainingDesign.id! });
  }, [selectedTrainingDesign]);
  ...
  useQuery({
    ...
    queryFn: async () => {
      ...
      const reducedFacilitators = newFacilitators
        .filter(({ id }) => !selectedFacilitators.some((e) => e.id === id))
        .sort((a, b) => (a.name > b.name ? 1 : -1));

      setFacilitators(reducedFacilitators);
      ...
      <form id="internalCourseOutlineForm" key="internalCourseOutlineForm" onSubmit={handleSubmit(onSubmit)}>
        <div className="mt-1">
          <div className="mt-1 mb-4">
            ...
            <div className="p-2 text-sm border rounded">{selectedTrainingDesign?.courseTitle}</div>
          </div>
        </div>
        <div className="mt-1">
          <div className="mb-2">
